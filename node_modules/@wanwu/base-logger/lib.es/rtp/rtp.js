...
const rtp = {
    ...
    getStartTime() {
        ...
        if (!obj.hasOwnProperty("loggerObj"))
            obj["loggerObj"] = {};
        let preStartTime = obj["loggerObj"]["__logStartTime"] ? obj["loggerObj"]["__logStartTime"] : localStorage.getItem("__logStartTime") ? localStorage.getItem("__logStartTime") : 0;
        crossUtil.setWindowName(obj);
        return preStartTime;
    },
    // 获取上报上一个页面的开始时间
    getPreStartTime() {
        ...
        if (!obj.hasOwnProperty("loggerObj"))
            obj["loggerObj"] = {};
        let preStartTime = obj["loggerObj"]["__logPreStartTime"] ? obj["loggerObj"]["__logPreStartTime"] : localStorage.getItem("__logPreStartTime") ? localStorage.getItem("__logPreStartTime") : 0;
        crossUtil.setWindowName(obj);
        return preStartTime;
    },
    // 获取本地缓存的loggerInfo
    getLoggerInfo() {
        ...
        if (!obj.hasOwnProperty("loggerObj"))
            obj["loggerObj"] = {};
        let loggerInfo = obj["loggerObj"]["__loggerInfo"] ? obj["loggerObj"]["__loggerInfo"] : localStorage.getItem("__loggerInfo") ? JSON.parse(localStorage.getItem("__loggerInfo")) : {};
        crossUtil.setWindowName(obj);
        return loggerInfo;
    },
    // 获取上报上一个页面的开始时间
    getEndTime() {
        ...
        if (!obj.hasOwnProperty("loggerObj"))
            obj["loggerObj"] = {};
        let endTime = obj["loggerObj"]["__logEndTime"] ? obj["loggerObj"]["__logEndTime"] : localStorage.getItem("__logEndTime") ? localStorage.getItem("__logEndTime") : 0;
        crossUtil.setWindowName(obj);
        return endTime;
    },
    ...
    // 获取cnt与ref
    getHashCntOrRefer(key) {
        let res = [getCntA(), getCntB()];
        // 模拟当前数据
        let jsonData = window.loggerData
            ? window.loggerData
            : {};
        if (key) {
            let keys = key.split("-");
