...
const send = {
    ...
    // CORS
    sendLogByCORS(url, logs, newLogFormat) {
        try {
            const param = {
                data: logs,
            };
    ...
    getLog() {
        ...
        const log__tasks = localStorage.getItem(this.localStorageKey) || '[]';
        return JSON.parse(log__tasks);
    },
    getNewLog() {
        // tslint:disable-next-line:variable-name
        const log__tasks = localStorage.getItem(this.NewlocalStorageKey) || '[]';
        return JSON.parse(log__tasks);
    },
    clearLog() {
    ...
    logImmediately() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const isErr = yield send.sendLog(logs);
            const isNewErr = yield send.sendNewLog(newLogs);
            if (isErr || isNewErr) {
    ...
    checkListLog(eventId, data) {
        ...
        if (eventId === 'listShow') {
            if (!data.domain) {
                ...
                console.warn('listShow埋点数据 domain格式有误！请参考埋点格式文档进行修改', data);
                data.domain = [Object.assign({}, data.domain)];
                return data;
            }
            else {
                return data;
            ...
            if (!data.domain) {
                ...
                console.warn('listShow埋点数据 domain格式有误！请参考埋点格式文档进行修改', data);
                data.domain = Object.assign({}, data.domain[0]);
                return data;
            }
            else {
                return data;
