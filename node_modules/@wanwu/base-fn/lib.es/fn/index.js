...
export const isEqualWithStack = (a, b, aStack = [], bStack = []) => {
    ...
    if (areArrays) {
        ...
        // 递归，这递归写的真丑
        while (lengthA--) {
            if (!isEqualWithStack(a[lengthA], b[lengthA], aStack, bStack))
                return false;
        }
    }
    else {
        ...
        while (keysALength--) {
            ...
            if (!(has(b, key) && isEqualWithStack(a[key], b[key], aStack, bStack)))
                return false;
        }
    }
...
export function setAt(path, root, data) {
    if (!path) {
        return;
    }
...
export function formatPriceDrawer(price, fractionDigits) {
    if (!price && price !== 0) {
        return '';
    }
    else if (typeof price === 'string') {
 ...
 * @return {string}
 */
export function stringifyUrl(url, params = {}, encode = true) {
    // 从url取的数据，一定需要decode
...
export function parseUrl(url, params = {}, decode = true) {
 ...
 * @return {string} 参数
 */
export function getQueryString(name, url = location.search) {
    if (!name)
        return '';
    url = url.split('#')[0];
 ...
 */
export function setQueryString(uri, key, value) {
 ...
 * @param key
 */
export function removeQueryString(uri, key) {
    const re = new RegExp('([?&])' + key + '=.*?(&|#|$)', 'i');
    const separator = uri.indexOf('?') !== -1 ? '&' : '?';
    let uriRes = uri;
 ...
 */
export function getFormatTime(leftSecond, format) {
...
export function filter(data, filterCallback) {
    if (isArray(data)) {
...
export const isLargerThanAppVersion = function (appVersion) {
...
export const clientColor2webColor = function (color) {
    if (typeof color !== "string")
        return color;
    if (color.startsWith('#') && color.length === 9) {
        ...
        const opacity = color.slice(1, 3);
        const colorVal = color.slice(3, 9);
        return colorConvert(`#${colorVal}${opacity}`);
    }
    else {
        return color;
    }
