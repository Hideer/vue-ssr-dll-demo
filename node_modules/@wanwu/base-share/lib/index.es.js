...
import { ua, ImageUtils, fn } from '@wanwu/base-fn';
import { createShareParams } from '@wanwu/base-logger';
...
var index = {
  ...
  getShareOptions: function getShareOptions() {
    ...
    var isPublicFlow = fn.getQueryString('isPublicFlow');

    if (isPublicFlow) {
      var _url = shareOptions.link;
      shareOptions.link = fn.setQueryString(_url, 'isPublicFlow', '0');
    } // 分享渠道


    var sf = fn.getQueryString('sf', shareOptions.link);

    if (!sf) {
      var _url2 = shareOptions.link;
      shareOptions.link = fn.setQueryString(_url2, 'sf', 'h5');
    ...
    ShareRemoveKeys.map(function (removeKey) {
      var val = fn.getQueryString(removeKey);
      if (!val) return;
      var url = shareOptions.link;
