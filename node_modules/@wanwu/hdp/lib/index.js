...
/**
 * 判断是否进行版本控制的操作
 * （现期版本内排除wwdz_b2b，也就是收藏集市）
 * @returns {boolean} true - 进行，false - 不进行
 */

function isToControlVersion() {
  return !['wwdz_b2b'].includes(baseFn.ua.getAppName());
}
...
function inject() {
  ...
  if (baseFn.ua.isIOSApp() && isToControlVersion() && baseFn.fn.compareVersion(baseFn.ua.getAppVersion(), '2.2.0') < 0) {
...
function exec(service, action) {
  ...
  return new Promise(function (resolve, reject) {
    // [FIX]是否进行版本控制的比较
    if (isIOSApp() && isToControlVersion() && baseFn.fn.compareVersion(baseFn.ua.getAppVersion(), '2.2.0') < 0) {
...
if (isToControlVersion() && baseFn.fn.compareVersion(baseFn.ua.getAppVersion(), '2.1.5') < 0) {
