...
var throughParamsPlugin = require('@wanwu/location-plugin-through-params');
var throughParamsPlugin__default = _interopDefault(throughParamsPlugin);
var eventUtils = require('@wanwu/event-utils');
var hdp = _interopDefault(require('@wanwu/hdp'));

var PREVENT_ERROR_TYPE = 'prevent';
var eventCenter = new baseFn.EventEmitter(); // 给插件提供beforeNavigate事件

eventCenter.$on('beforeNavigate', callBeforeNavigateList);
var tarsksObj = {
  // 插件列表
  pluginList: [baseLogger.createRtpParams, throughParamsPlugin__default],
  // 跳转hook列表
  beforeNavigateList: []
};
var throttleDoMethod = baseFn.fn.throttle(doMethod, 300, {
  trailing: false
});
var wLocation = {
  // vue router实例
  ...
  setConfig: function setConfig(options) {
    ...
    var newOption = baseFn.fn.pick(options, pickKeys);
...
eventUtils.on(document.body, 'click', handleAClick);
...
function callBeforeNavigateList(to, from) {
  tarsksObj.beforeNavigateList.forEach(function (beforeNavigateFn) {
    return beforeNavigateFn(baseFn.fn.clone(to), baseFn.fn.clone(from));
  });
}
...
function prevent(options, THROW) {
  ...
  if (baseFn.fn.getTypeof(options) === 'string') {
    err.message = options || err.message;
  }

  if (baseFn.fn.getTypeof(options) === 'object') {
...
function pathMaker(path, keepState) {
  ...
  if (path && typeof path === 'string') {
    var pathObj = baseFn.fn.parseUrl(path);
    newPathObj = {
      path: pathObj.path,
      query: pathObj.params || {},
      hash: pathObj.hash
    };
  } else if (baseFn.fn.getTypeof(path) === 'object') {
    var _pathObj = baseFn.fn.parseUrl(path.path || '');
  ...
  if (/^\/.+/.test(newPathObj.path) && baseFn.fn.typeYalidator.isEmpty(newPathObj.origin) && baseFn.fn.typeYalidator.isEmpty(newPathObj.base)) {
    if (wLocation.router) {
      newPathObj.base = wLocation.router.history.base;
    }
  }

  if (/^\/.+/.test(newPathObj.path) && baseFn.fn.typeYalidator.isEmpty(newPathObj.origin)) {
          ...
          case 0:
            ...
            tarsksObj.pluginList.forEach(function (plugin) {
              pluginsPromise = pluginsPromise.then(function (to) {
                if (!to) return false;
                return plugin(pathMaker(to), baseFn.fn.clone(from), prevent, eventCenter, options, method);
              });
            });
            _context6.next = 7;
      ...
      while (1) {
        switch (_context7.prev = _context7.next) {
          case 0:
            ...
            newPath = baseFn.fn.stringifyUrl("".concat(to.origin || '').concat(to.base && to.base !== '/' ? to.base : '').concat(to.path || ''), to.query) + (to.hash ? "#".concat(to.hash) : '');
            openWebview = options.openWebview;
            hasOpenWebviewOptions = !baseFn.fn.typeYalidator.isUndefined(openWebview);
            urlOpenWebview = baseFn.fn.getQueryString('__openWebview') === 'true';
          ...
          case 22:
            ...
            return _context7.abrupt("return", wLocation.router[method](baseFn.fn.pick(to, ['path', 'name', 'query', 'params', 'hash'])));
...
exports.throughParams = throughParamsPlugin;
exports.default = wLocation;
