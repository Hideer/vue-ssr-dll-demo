...
var getLazyFn = function getLazyFn() {
  ...
  if (baseFn.fn.getQueryString('hidewwdz') === '1') {
    // 初始化时赋值到全局 给其他组件使用
  ...
  var addListener = function addListener(ele, binding, vnode) {
    ...
    if (targetImgSize && targetImgSize.width && targetImgSize.height) {
      ...
      item.loading = baseFn.ImageUtils.getRectangleImage(loadingImgObj.url, {
        cut: true,
        width: loadingWdith,
      ...
      item.error = baseFn.ImageUtils.getRectangleImage(perchImg.error, {
        cut: true,
        width: errorWdith,
        height: errorHeight
      });
...
function getDefaultSize(src) {
  var origin = baseFn.ImageUtils.getOriginImage(src);
