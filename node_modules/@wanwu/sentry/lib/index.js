...
var errorClass_1 = __importDefault(require("./errorClass"));
...
var wanwuSentry = function () {
     ...
     * 低版本不支持的浏览器不上报
     */
    var captureExceptionFilterForBrowser = function () {
     ...
     * @param value
     */
    var isMatchedIgnoreRules = function (value, arr) {
        if (!value) {
            return false;
        }
        var ignoreArr = ignoreRules;
        if (arr) {
    ...
    // 设置上自己的错误类型
    var setMySentryError = function (opt) {
        var cError = new errorClass_1.default(opt);
        return cError;
    };
    // 全局捕获的错误API
    var fnErrorTrap = function (err) {
        setError(err);
    };
    // 设置用户相关
    var setUserOrTag = function (opt) {
    ...
    var setConfig = function (opt) {
        var wx = opt.wx, window = opt.window, vue = opt.vue;
        if (myEnv === 'vue' && vue) {
    ...
    return {
        init: init,
        logError: logError,
        setMySentryError: setMySentryError,
        setUserOrTag: setUserOrTag,
        ERROR_TYPE: utils_1.ERROR_TYPE,
        ERROR_LEVEL: utils_1.ERROR_LEVEL,
    };
};
exports.default = wanwuSentry();
