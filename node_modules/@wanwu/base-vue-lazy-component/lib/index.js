...
var logE = baseLogger.logE;
var throttle = baseFn.fn.throttle;
...
var onListenScroll = function onListenScroll(vm) {
  ...
  if (!isWindowBind) {
    DEFAULT_EVENTS.forEach(function (evt) {
      eventUtils.on(window, evt, onScroll, true); // window.addEventListener(evt, onScroll);
    });
    eventUtils.on(document, 'visibilitychange', onVisible);
    isWindowBind = true;
  ...
  exposureLsit.forEach(function (item) {
    ...
    if (item.newExposureData) {
      ...
      if (event === 'listShow') {
        ...
        _dataObj = baseFn.fn.omit(item.newExposureData, 'event');
      }
...
function addListerner(vm) {
  ...
  if (!dataObj[vm.type]) {
    dataObj[vm.type] = {
      listenList: [],
      showScope: vm.showScopeCache ? baseFn.fn.clone(vm.showScopeCache) : {
        0: 0,
        1: 200
      }
    };
  ...
  if (dataObj[vm.type].listenList.indexOf(vm) === -1) {
    ...
    if (catIn) {
      dataObj[vm.type].showScope = vm.showScopeCache ? baseFn.fn.clone(vm.showScopeCache) : {
...
function cacheShowScope(type) {
  ...
  if (dataObj[type]) {
    showScopeCache = baseFn.fn.clone(dataObj[type].showScope);
  }
